...
<main class="p1-main">
  ...
  {card_block(
    ...
    settings_row('shield-checkmark-outline', '#33A089', 'Confidentialité &amp; consentement') +
...
<main class="p1-main">
  ...
  {card_block(
    ...
    settings_row('chatbubble-outline', '#33A089', 'Messages directs', control=toggle_on(True))
  )}
...
<main class="p1-main">
  ...
  <div class="p1-mfa-card">
    <span class="p1-mfa-ic" style="background:rgba(51,160,137,0.14);color:{MINT};box-shadow:0 0 18px rgba(51,160,137,0.15);filter:drop-shadow(0 0 10px rgba(51,160,137,0.15))">
  ...
  <div class="p1-sub-hero">
    <div class="p1-sub-hero-row">
      <div>
        ...
        <div class="p1-sub-hero-name">Premium · Mensuel</div>
      </div>
      <span class="p1-sub-hero-badge">ACTIF</span>
    </div>
    <div class="p1-sub-hero-price">99 $ <span>/ mois</span></div>
...
<main class="p1-main">
  ...
  <div class="p1-pro-hero">
    {smuppy_icon(72, 'gradient' if th['html']=='dark' else 'dark')}
    <h1 class="p1-pro-title">Pro Creator Premium</h1>
    <p class="p1-pro-sub">Débloque la monétisation créateur</p>
  </div>

  <div class="p1-plans">
    <div class="p1-plan p1-plan-active" style="grid-column:1 / -1">
      <div class="p1-plan-header">
        <span class="p1-plan-badge">TON COMPTE · PRO CREATOR</span>
      </div>
      <div class="p1-plan-name">Pro Creator Premium</div>
      <div class="p1-plan-price">99 $ <span>/ mois</span></div>
      <div class="p1-plan-equiv">Sans engagement · annulable à tout moment</div>
    </div>
  </div>

  <h3 class="p1-sec-title">Ce que tu débloques</h3>
  {card_block(
    settings_row('videocam-outline', '#E63946', 'Aller en live', sub='Diffusion Agora HD + lives payants') +
    settings_row('play-circle-outline', MINT, 'Abonnements de chaîne', sub='Tes fans s&rsquo;abonnent au prix que tu fixes') +
    settings_row('gift-outline', '#FFA63D', 'Recevoir des Smups', sub='Pourboires des fans + Creator Wallet') +
    settings_row('videocam', '#11C6FF', 'Sessions 1:1 &amp; packs', sub='Coaching vidéo payant (tu gardes 80%)') +
    settings_row('calendar-outline', '#9966FF', 'Events &amp; groupes illimités', sub='+ events payants') +
    settings_row('school-outline', '#33A089', 'Cours VOD (Smuppy Train)', sub='Vends tes programmes')
  )}

  <div class="p1-form" style="padding:0">
    {primary_cta("S&rsquo;abonner · 99,99 $/mois")}
    <p class="p1-trial-note">Le badge vérifié est un abonnement séparé (14,99 $/mois). Facturé via l&rsquo;App Store.</p>
  </div>
</main>'''
...
<main class="p1-main">
  ...
  <div class="p1-issue-grid">
    ...
    <button class="p1-issue">
      <ion-icon name="speedometer-outline" style="font-size:22px;color:#33A089"></ion-icon>
      <span>Perf</span>
    </button>
    <button class="p1-issue">
...
def screen_notifications(th: dict) -> str:
    A = lambda i: f'https://images.unsplash.com/photo-{i}?w=80&h=80&fit=crop&q=80'
    # (avatar_ids[list], type_icon, type_color, name, action, time, read, photo)
    notifs = [
        (['1494790108377-be9c29b29330', '1507003211169-0a1dd7228f2d', '1531746020798-e6953c6e8e04'],
         'heart', '#E63946', 'Emma Chen + 4 autres', 'ont aimé ta photo', '5 min', False, 'photo'),
        (['1507003211169-0a1dd7228f2d'], 'person-add', '#11C6FF', 'Marc Dubois', 'est devenu un fan', '1 h', False, None),
        (['1531746020798-e6953c6e8e04'], 'star', MINT, 'Léa Martin', "s'est abonnée à ta chaîne", '2 h', False, None),
        (['1502086223501-7ea6ecd79368'], 'chatbox', '#9966FF', 'Thomas Roy', "t'a envoyé un message", '3 h', True, None),
        (['1500648767791-00dcc994a43e'], 'cash', '#FFA63D', 'Coach Mike', "t'a envoyé 50 👊 Smups", '6 h', True, None),
        (['1494790108377-be9c29b29330'], 'videocam', '#E63946', 'Sara Khan', 'est en live', 'Hier', True, None),
        (['1535713875002-d1d0cf377fde'], 'person-add', '#11C6FF', 'Emma Chen', 'a demandé à être ton fan', 'Hier', True, None),
    ...
    for date, group in by_date.items():
        ...
        for avatars, icon, hue, name, action, time, read, _photo in group:
            read_cls = ' p1-notif-read' if read else ''
            photo_html = ('<img src="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=80&h=80&fit=crop&q=80" alt="" class="p1-notif-photo"/>'
                          if _photo else '')
            if avatars:
                avs = ''.join(
                    f'<img class="p1-na" style="margin-left:{-12 if j else 0}px;z-index:{len(avatars)-j}" src="{A(a)}" alt=""/>'
                    for j, a in enumerate(avatars[:3]))
                lead = (f'<span class="p1-na-stack">{avs}'
                        f'<span class="p1-na-badge" style="background:{hue}"><ion-icon name="{icon}" style="font-size:10px;color:#fff"></ion-icon></span></span>')
            else:  # system/activity notification → colored icon (no actor)
                lead = (f'<span class="p1-notif-ic" style="background:{hue}14;color:{hue}">'
                        f'<ion-icon name="{icon}" style="font-size:18px;color:{hue}"></ion-icon></span>')
...
<style>
  ...
  .p1-card { background:var(--card);
    border:1px solid rgba(51,160,137,0.12);
    border-radius:18px; padding:4px 4px;
    box-shadow:0 0 16px rgba(51,160,137,0.05); }
  ...
  .p1-tog-on { background:var(--mint); box-shadow:0 0 12px rgba(51,160,137,0.225); }
  ...
  .p1-profile-card { display:flex; align-items:center; gap:14px;
    ...
    background:var(--card); border:1.5px solid rgba(51,160,137,0.15);
    border-radius:22px;
  ...
  .p1-profile-avatar { width:60px; height:60px; border-radius:999px; object-fit:cover;
    ...
    box-shadow:0 0 12px rgba(51,160,137,0.15); }
  ...
  .p1-profile-badge { display:inline-block; padding:3px 8px; border-radius:999px;
    background:rgba(51,160,137,0.16); color:var(--mint);
  ...
  .p1-mfa-card { display:flex; align-items:center; gap:14px;
    ...
    background:linear-gradient(135deg, rgba(51,160,137,0.12), rgba(0,179,199,.04));
    border:1.5px solid rgba(51,160,137,0.175);
    border-radius:22px;
  ...
  /* Subscription hero */
  .p1-sub-hero {
    background:linear-gradient(135deg, rgba(51,160,137,0.18), rgba(0,179,199,.10));
    border:1.5px solid rgba(51,160,137,0.225);
    border-radius:24px; padding:18px;
    box-shadow:0 0 32px rgba(51,160,137,0.22);
    margin:0 0 14px; }
  ...
  .p1-sub-hero-badge { padding:4px 10px; border-radius:999px;
    background:rgba(51,160,137,0.15); color:var(--mint);
  ...
  .p1-sub-hero-btn { width:100%; margin-top:14px; padding:11px;
    border:1.5px solid rgba(51,160,137,0.225); border-radius:14px;
    background:rgba(51,160,137,0.08); color:var(--mint);
    font-family:inherit; font-size:13px; font-weight:700; cursor:pointer;
    box-shadow:0 0 14px rgba(51,160,137,0.12); }

  /* Upgrade hero + plans */
  .p1-pro-hero { display:flex; flex-direction:column; align-items:center;
    text-align:center; gap:8px; margin:8px 0 18px;
    filter:drop-shadow(0 0 24px rgba(51,160,137,0.2)); }
  ...
  .p1-plan { flex:1; padding:14px; border-radius:18px;
    ...
    position:relative; }
  .p1-plan-active { border-color:rgba(51,160,137,0.225);
    background:rgba(51,160,137,0.06);
    box-shadow:0 0 20px rgba(51,160,137,0.2); }
  ...
  .p1-issue-on { border-color:rgba(51,160,137,0.225); background:rgba(51,160,137,0.06);
  ...
  .p1-ss-add { width:72px; height:72px; border-radius:14px;
    background:var(--input-bg); border:1.5px dashed rgba(51,160,137,0.225);
  ...
  .p1-msg-row:hover { background:rgba(51,160,137,0.04); }
  ...
  .p1-msg-online { position:absolute; bottom:0; right:0;
    ...
    box-shadow:0 0 6px rgba(51,160,137,0.225); }
  ...
  .p1-msg-unread { background:var(--mint); color:#fff;
    ...
    box-shadow:0 0 10px rgba(51,160,137,0.2); flex-shrink:0; }
  .p1-msg-urgent { background:rgba(51,160,137,0.04); border-left:3px solid var(--mint); }
  ...
  .p1-chat-input-send { width:36px; height:36px; border:none; border-radius:999px;
    ...
    display:flex; align-items:center; justify-content:center;
    box-shadow:0 4px 12px rgba(51,160,137,0.2); }
  ...
  .p1-new-group { display:flex; align-items:center; gap:12px;
    ...
    background:rgba(51,160,137,0.08); border:1.5px solid rgba(51,160,137,0.15);
  ...
  .p1-new-group-ic { width:36px; height:36px; border-radius:11px;
    background:rgba(51,160,137,0.16);
    display:inline-flex; align-items:center; justify-content:center;
    flex-shrink:0; }
  ...
  .p1-mb-pill-me { background:linear-gradient(135deg, var(--mint-hi), var(--mint-deep)); color:#fff;
    border:none; padding:6px 12px; box-shadow:0 0 10px rgba(51,160,137,0.175); }
  ...
  .p1-mb-add { display:inline-flex; align-items:center; gap:6px;
    ...
    background:rgba(51,160,137,0.08); border:1.5px dashed rgba(51,160,137,0.225);
  ...
  .p1-notif-ic { width:36px; height:36px; border-radius:11px;
    display:inline-flex; align-items:center; justify-content:center;
    flex-shrink:0; }
  /* avatars de profil empilés + badge type (founder jun03) */
  .p1-na-stack { position:relative; display:inline-flex; align-items:center; flex-shrink:0; padding-bottom:2px; }
  .p1-na { width:38px; height:38px; border-radius:999px; object-fit:cover;
    border:2px solid var(--page); }
  .p1-na-badge { position:absolute; bottom:-2px; right:-2px; width:18px; height:18px; border-radius:999px;
    display:flex; align-items:center; justify-content:center; border:2px solid var(--page); z-index:10; }
  ...
  .p1-req-btn-accept { background:linear-gradient(135deg, var(--mint-hi), var(--mint-deep));
    color:#fff; box-shadow:0 4px 12px rgba(51,160,137,0.2); }
  ...
  .p1-fan-btn { padding:7px 14px; border:1.5px solid rgba(51,160,137,0.225);
    border-radius:999px; background:rgba(51,160,137,0.06);
    color:var(--mint); font-family:inherit; font-size:12.5px; font-weight:700;
    cursor:pointer;
    box-shadow:0 0 12px rgba(51,160,137,0.15); }
  .p1-fan-btn-on { padding:7px 14px; border-radius:999px;
    background:rgba(51,160,137,0.14); border:1.5px solid rgba(51,160,137,0.2);
    color:var(--mint); font-family:inherit; font-size:12.5px; font-weight:700;
  ...
  .p1-cc-card { position:relative; width:240px; aspect-ratio:5/7;
    ...
    box-shadow:0 0 40px rgba(51,160,137,0.2),
  ...
  .p1-correction-banner { display:flex; gap:10px;
    ...
    background:rgba(51,160,137,0.08);
