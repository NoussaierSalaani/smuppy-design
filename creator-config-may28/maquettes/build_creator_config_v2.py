...
V2 foundation (tokens.md, brand-rules.md):
  - mint solid #26C1A4 (95% UI), mint-hi #4EDCBE (dark accent)
...
def bottomnav_canonical(active: str = 'home') -> str:
    """Canonical bottom nav matching v2_canonical_home_feed (founder may29 23:58 lock).
    Slots: home / play (peaks+live) / create+ / messagerie / profile (avatar SQUARE + mint frame).
    Active state: home/play/msg = mint fill icon + active-bar. profile = mint outline + glow.
    Center create = pill-active-dark (card bg + mint border + mint icon halo) — NOT gradient fill.
    """
    def _slot(key, icon, label):
        on = (active == key)
        glow = ' bnav-on' if on else ''
        bar = '<div class="bnav-bar"></div>' if on else ''
        ico = f'<ion-icon name="{icon}" style="font-size:24px"></ion-icon>'
        return f'<button class="bnav-item{glow}" aria-label="{label}">{ico}{bar}</button>'
    avatar_cls = 'bnav-profile bnav-profile-on' if active == 'profile' else 'bnav-profile'
    return f'''<nav class="bnav-canonical">
  {_slot('home', 'home', 'Home')}
  {_slot('play', 'play-circle-outline', 'Play')}
  <button class="bnav-create" aria-label="Create"><ion-icon name="add" style="font-size:24px"></ion-icon></button>
  {_slot('msg', 'chatbubble-outline', 'Messages')}
  <button class="{avatar_cls}" aria-label="Profile"><img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=80&h=80&fit=crop&q=80" alt=""/></button>
</nav>'''
...
def popup_channel_price_picker(current: str = '9,99') -> str:
    """Channel setup · price picker — Apple IAP tiers predefined in code."""
    # Standard Apple IAP price tiers EUR
    tiers = [
        ('1,99', 'Tier 2 · découverte'),
        ('2,99', 'Tier 3 · essai'),
        ('4,99', 'Tier 5 · accessible'),
        ('9,99', 'Tier 10 · standard'),
        ('14,99', 'Tier 15 · premium'),
        ('19,99', 'Tier 20 · expert'),
        ('29,99', 'Tier 30 · elite'),
        ('49,99', 'Tier 50 · masterclass'),
    ]
    rows = ''
    for price, label in tiers:
        active = price == current
        cls = 'price-tier-row price-tier-on' if active else 'price-tier-row'
        check = '<ion-icon name="checkmark-circle" style="font-size:20px;color:#26C1A4"></ion-icon>' if active else ''
...
def popup_flow_success(sheet_id: str, emoji: str, title: str, body: str, primary: str, secondary: str = 'Fermer') -> str:
  ...
  <div class="sheet">
    <div class="sheet-handle"></div>
    <div class="sheet-icon-wrap success">
      <ion-icon name="checkmark-circle" style="font-size:38px;color:#26C1A4"></ion-icon>
    </div>
...
def popup_success(screen_title: str, action: str, t: dict) -> str:
  ...
  <div class="sheet">
    <div class="sheet-handle"></div>
    <div class="sheet-icon-wrap success">
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#26C1A4" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
    </div>
    <h3 class="sheet-title">{title}</h3>
    <p class="sheet-body">{body}</p>
    <div class="sheet-actions sheet-actions-stacked">
...
<section class="cv-perks-list" style="padding-bottom:14px">
  ...
  <div class="cv-perk-row"><span class="cv-perk-icon" style="background:rgba(38,193,164,.12);color:#26C1A4"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#26C1A4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg></span><div class="cv-perk-text"><h4>Lives exclusifs</h4><p>Accès à tous les lives + replays</p></div></div>
...
def screen_1on1_owner_profile(t: dict) -> tuple:
  ...
  <div class="owner-banner-icon" style="background:rgba(38,193,164,.16);color:#26C1A4">
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#26C1A4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="filter:drop-shadow(0 0 16px rgba(38,193,164,.4))"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>
  </div>
...
<section style="padding:0 16px">
  ...
  <div class="pack-card">
    <div class="pack-card-thumb" style="background:linear-gradient(135deg,#A8E6D9,#26C1A4)">
      <svg width="22" height="22" viewBox="0 0 24 24" fill="#fff" stroke="#fff" stroke-width="0.5"><path d="M12 2l2.39 7.36H22l-6.19 4.5 2.37 7.32L12 16.66l-6.18 4.52 2.37-7.32L2 9.36h7.61z"/></svg>
    </div>
    <div class="pack-card-meta">
...
def screen_1on1_view(t: dict) -> tuple:
    """FAN-FACING 1:1 booking view — visitor picks duration + slot."""
    # Compact non-scroll layout: hero avatar + creator name + duration picker + date+time picker + book CTA
  ...
  <div class="bv-avatar-wrap">
    ...
    <span class="bv-verified">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="#26C1A4" stroke="#fff" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>
    </span>
  </div>
...
<section class="bv-section">
  <h3 class="sec">Durée</h3>
  <div class="bv-durs">
    <button class="bv-dur"><span>15</span><em>min · 15 €</em></button>
    <button class="bv-dur bv-dur-on"><span>30</span><em>min · 28 €</em></button>
    <button class="bv-dur"><span>60</span><em>min · 45 €</em></button>
    <button class="bv-dur"><span>90</span><em>min · 69 €</em></button>
  </div>
</section>
<section class="bv-section">
  <h3 class="sec">Date</h3>
  <div class="bv-week-row">
    <button class="bv-day"><span>Lun</span><b>2</b></button>
    <button class="bv-day"><span>Mar</span><b>3</b></button>
    <button class="bv-day"><span>Mer</span><b>4</b></button>
    <button class="bv-day bv-day-on"><span>Jeu</span><b>5</b></button>
    <button class="bv-day"><span>Ven</span><b>6</b></button>
    <button class="bv-day"><span>Sam</span><b>7</b></button>
    <button class="bv-day bv-day-disabled"><span>Dim</span><b>8</b></button>
  </div>
  <button class="bv-date-pick">
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#26C1A4" stroke-width="2" stroke-linecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
    <span>Voir un autre mois</span>
  </button>
</section>
<section class="bv-section">
  <h3 class="sec">Créneaux · jeudi 5 juin</h3>
  <div class="bv-times">
    <button class="bv-time">09:00</button>
    <button class="bv-time">10:30</button>
    <button class="bv-time bv-time-on">14:00</button>
    <button class="bv-time">15:30</button>
    <button class="bv-time">17:00</button>
    <button class="bv-time bv-time-off">18:30</button>
  </div>
</section>
<section class="bv-section">
  <h3 class="sec">Sujet de la session · optionnel</h3>
  <div class="bv-subject-wrap">
    <textarea class="bv-subject" rows="3" placeholder="Décris ton objectif · ex. revue de programme, ajustements nutrition, objectifs du mois…"></textarea>
    <span class="bv-subject-count">0 / 240</span>
  </div>
</section>'''
    return body, "Réserver · 28 €"
...
<section class="cv-perks-list" style="padding-bottom:14px">
  <div class="cv-perk-row">
    <span class="cv-perk-icon" style="background:rgba(38,193,164,.12);color:#26C1A4">
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#26C1A4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2"/></svg>
    </span>
...
<section class="cv-overlap-card">
  ...
  <div class="cv-premium-pill">
    ...
    <div class="cv-premium-star">
      <ion-icon name="star" style="font-size:18px;color:#26C1A4"></ion-icon>
    </div>
  </div>
...
def screen_channel_view_subscribed(t: dict) -> tuple:
  ...
  <ion-icon name="checkmark-circle" style="font-size:18px;color:#26C1A4"></ion-icon>
...
def screen_1on1_view_booked(t: dict) -> tuple:
  ...
  <div class="confirm-check-wrap">
    ...
    <div class="confirm-check-inner">
      <ion-icon name="checkmark" style="font-size:30px;color:#26C1A4"></ion-icon>
    </div>
  </div>
...
<section class="confirm-card">
  ...
  <div class="confirm-grid">
    <div class="confirm-grid-col">
      <div class="confirm-grid-icon"><ion-icon name="calendar-outline" style="font-size:18px;color:#26C1A4"></ion-icon><span>DATE</span></div>
      <span class="confirm-grid-val">Jeudi 5<br>Juin</span>
    </div>
    <div class="confirm-grid-col">
      <div class="confirm-grid-icon"><ion-icon name="time-outline" style="font-size:18px;color:#26C1A4"></ion-icon><span>HEURE</span></div>
...
def _profile_shell_body(is_owner: bool) -> tuple:
    ...
    if is_owner:
        cta_main = '<button class="profile-follow-btn">Modifier</button>'
        cta_icon = '<button class="profile-msg-icon" aria-label="Partager"><ion-icon name="share-social-outline" style="font-size:18px"></ion-icon></button>'
        cta_label = 'Statistiques'
    else:
        cta_main = '<button class="profile-follow-btn">Follow</button>'
        cta_icon = '<button class="profile-msg-icon" aria-label="Message"><ion-icon name="mail-outline" style="font-size:18px"></ion-icon></button>'
...
def screen_post_success(t: dict) -> tuple:
    """Confirmation : big mint check halo + Post publié + 2 CTAs + status mini-card."""
    body = f'''<div class="ps-wrap">
  <div class="ps-particles"></div>
  <div class="ps-check">
    <div class="ps-check-ring"></div>
    <ion-icon name="checkmark" class="ps-check-icon"></ion-icon>
  </div>
  <h2 class="ps-title">Post publié&nbsp;!</h2>
  <p class="ps-body">Ta tribu peut maintenant voir ta nouvelle publication.</p>
  <div class="ps-actions">
    <button class="ps-cta-primary">Voir mon post</button>
    <button class="ps-cta-ghost">Retour au fil d'actualité</button>
  </div>
  <div class="ps-status-card">
    <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=120&h=120&fit=crop&q=80" alt="thumb"/>
    <div class="ps-status-text">
      <div class="ps-status-label">Statut de la publication</div>
      <div class="ps-status-live"><span class="ps-live-dot"></span>EN LIGNE</div>
    </div>
  </div>
</div>'''
    return body, None
...
SCREENS = {
    ...
    'creator_profile_owner_v2': ('Sara Khan', screen_profile_owner),
    'creator_profile_visitor_v2': ('Sara Khan', screen_profile_visitor),
...
<style>
  ...
  * {{ box-sizing:border-box; -webkit-font-smoothing:antialiased; -webkit-tap-highlight-color:transparent; }}
  ...
  .back:active {{ background:rgba(38,193,164,.10); }}
  ...
  .hero-compact-card {{ display:flex; align-items:center; gap:14px;
    ...
    background:linear-gradient(135deg, rgba(38,193,164,.10), rgba(0,179,199,.06));
    border:1px solid rgba(38,193,164,.18); }}
  .hero-compact-icon {{ width:48px; height:48px; border-radius:14px;
    background:rgba(38,193,164,.16); display:flex; align-items:center; justify-content:center;
    flex-shrink:0;
    box-shadow:0 0 24px rgba(38,193,164,.30); }}
  ...
  .card {{ background:var(--card); border-radius:24px; overflow:hidden;
    border:1.5px solid rgba(38,193,164,.18);
    box-shadow:0 0 24px rgba(38,193,164,.10),
               0 0 0 1px rgba(38,193,164,.06) inset,
  ...
  .val.mint {{ color:var(--mint); font-weight:700; border-color:rgba(38,193,164,.30);
  ...
  .add-row:hover {{ background:rgba(38,193,164,.06); }}
  /* ── Durations (1:1) ──────────────────────────────────────── */
  ...
  .ta:focus {{ border-color:var(--mint); box-shadow:0 0 0 3px rgba(38,193,164,.18); }}
  ...
  .tog.on {{ background:var(--mint); box-shadow:0 0 16px rgba(38,193,164,.45); }}
  ...
  .cta {{ width:100%; height:54px; border:none; border-radius:999px;
    ...
    box-shadow:0 8px 24px rgba(38,193,164,.45), 0 0 0 1px rgba(255,255,255,.06) inset; }}
  ...
  .cv-premium-tag {{ position:absolute; bottom:16px; left:16px; z-index:2;
    ...
    box-shadow:0 4px 16px rgba(38,193,164,.45);
  }}
  ...
  .cv-overlap-card {{ position:relative; z-index:5;
    ...
    border:1px solid rgba(38,193,164,.15);
    box-shadow:0 14px 40px rgba(0,0,0,{t['dim_shadow']}),
               0 0 0 1px rgba(38,193,164,.08),
               0 0 32px rgba(38,193,164,.10);
    text-align:center;
  }}
  ...
  .cv-premium-pill {{ display:flex; align-items:center; justify-content:space-between;
    ...
    background:var(--input-bg); border:1.5px solid rgba(38,193,164,.30);
    box-shadow:0 0 18px rgba(38,193,164,.12);
  ...
  .cv-premium-star {{ width:36px; height:36px; border-radius:999px;
    background:rgba(38,193,164,.12); display:flex; align-items:center; justify-content:center;
    flex-shrink:0; box-shadow:0 0 16px rgba(38,193,164,.30); }}
  ...
  .cv-check-small {{ flex-shrink:0; width:20px; height:20px; border-radius:999px;
    ...
    box-shadow:0 2px 6px rgba(38,193,164,.30); }}
  ...
  .prof-head {{ display:flex; align-items:flex-end; justify-content:space-between;
    padding:0 16px; margin-top:-30px; gap:10px; }}
  .prof-avatar {{ position:relative; }}
  .prof-avatar img {{ width:72px; height:72px; border-radius:999px; object-fit:cover;
    border:3.5px solid var(--page); box-shadow:0 4px 12px rgba(0,0,0,.25); }}
  .prof-verified {{ position:absolute; bottom:0; right:0; width:20px; height:20px;
  ...
  /* Actions row (Become a fan + msg icon) below stats — canonical may29 19:21 */
  ...
  .prof-follow-btn {{ height:36px; padding:0 20px; border:none; border-radius:999px;
    ...
    cursor:pointer; box-shadow:0 4px 14px rgba(38,193,164,.35); }}
  .prof-msg-icon {{ width:36px; height:36px;
    ...
    box-shadow:0 0 18px rgba(38,193,164,.30),
               0 0 0 1px rgba(38,193,164,.15) inset;
    filter:drop-shadow(0 0 6px rgba(38,193,164,.40)); }}
  .prof-meta {{ padding:14px 16px 12px; }}
  .prof-name {{ font-size:17px; font-weight:800; margin:0 0 2px;
    color:var(--text); letter-spacing:-.01em; }}
  .prof-role {{ font-size:13px; font-weight:500; color:var(--sub); margin:0 0 12px; }}
  ...
  .prof-main-pill-on {{
    ...
    box-shadow:0 0 16px rgba(38,193,164,.35),
               0 0 0 1px rgba(38,193,164,.45),
  ...
  .prof-sub-pill-on {{ background:var(--card); color:var(--mint);
    box-shadow:0 0 14px rgba(38,193,164,.30),
               0 0 0 1px rgba(38,193,164,.40),
  ...
  .prof-slot {{ padding:13px 0; border-radius:12px;
    ...
    cursor:pointer; transition:.18s ease; }}
  .prof-slot-on {{ background:rgba(38,193,164,.12); border-color:var(--mint); color:var(--mint);
    box-shadow:0 4px 14px rgba(38,193,164,.22); }}
  .prof-slot-off {{ color:var(--sub); text-decoration:line-through; opacity:.45; cursor:not-allowed; }}
  /* ── Canonical bottom nav (v2_canonical_home_feed parity — founder may29 lock) */
  .bnav-canonical {{
  ...
  .bnav-item.bnav-on {{ color:var(--mint);
    filter:drop-shadow(0 0 6px rgba(38,193,164,.55)); }}
  ...
  .bnav-create {{ display:flex; align-items:center; justify-content:center;
    width:48px; height:48px; border:1.5px solid rgba(38,193,164,.45);
    border-radius:999px; background:var(--card); color:var(--mint);
    cursor:pointer;
    box-shadow:0 0 16px rgba(38,193,164,.35),
               0 4px 12px rgba(38,193,164,.20),
               0 0 0 1px rgba(38,193,164,.15) inset;
    filter:drop-shadow(0 0 6px rgba(38,193,164,.40)); }}
  ...
  .bnav-profile.bnav-profile-on img {{ opacity:1;
    ...
    box-shadow:0 0 8px rgba(38,193,164,.45); }}
  ...
  .profile-cta-primary {{ flex:1; height:40px; border:none;
    ...
    cursor:pointer; box-shadow:0 4px 14px rgba(38,193,164,.30); }}
  ...
  .manage-chip-inline {{
    ...
    background:rgba(38,193,164,.12); color:var(--mint);
  ...
  .owner-banner {{
    ...
    background:var(--card); border:1px solid rgba(38,193,164,.15);
    box-shadow:0 0 0 1px rgba(38,193,164,.05),
               0 0 22px rgba(38,193,164,.08);
  }}
  ...
  .upcoming-row {{
    ...
    background:var(--card); border:1.5px solid rgba(38,193,164,.18);
    box-shadow:0 0 18px rgba(38,193,164,.08),
  ...
  .upcoming-status-ok {{ background:rgba(38,193,164,.12); color:var(--mint); }}
  ...
  .bv-dur em {{ font-size:10px; font-weight:600; color:var(--sub); font-style:normal;
    letter-spacing:.02em; }}
  .bv-dur-on {{ background:rgba(38,193,164,.10); border-color:var(--mint);
    box-shadow:0 4px 14px rgba(38,193,164,.22); }}
  ...
  .bv-day-on {{ background:rgba(38,193,164,.10); border-color:var(--mint);
    box-shadow:0 4px 14px rgba(38,193,164,.22); }}
  ...
  .bv-time {{ padding:12px 0; border-radius:12px;
    ...
    cursor:pointer; transition:.18s ease; }}
  .bv-time-on {{ background:rgba(38,193,164,.10); border-color:var(--mint); color:var(--mint);
    box-shadow:0 4px 14px rgba(38,193,164,.22); }}
  ...
  .bv-date-pick {{ width:100%; display:flex; align-items:center; justify-content:center;
    ...
    background:rgba(38,193,164,.06); border:1px dashed rgba(38,193,164,.35);
  ...
  .bv-subject {{ width:100%; padding:14px 16px; border-radius:18px;
    background:var(--card); border:1.5px solid rgba(38,193,164,.20);
    box-shadow:0 0 18px rgba(38,193,164,.10);
  ...
  .bv-subject:focus {{ border-color:var(--mint);
    box-shadow:0 0 24px rgba(38,193,164,.25), 0 0 0 3px rgba(38,193,164,.15); }}
  ...
  .sheet-icon-wrap.success {{
    background:rgba(38,193,164,.12);
    box-shadow:0 0 24px rgba(38,193,164,.40);
  }}
  ...
  .sheet-btn-primary {{
    background:linear-gradient(135deg, {GRAD_A} 0%, {GRAD_B} 100%);
    color:#fff;
    box-shadow:0 6px 20px rgba(38,193,164,.45);
  }}
  .sheet-btn-ghost {{
    background:transparent; color:var(--sub);
    border:1px solid var(--input-border);
  ...
  .price-tier-row {{ display:flex; align-items:center; justify-content:space-between;
    ...
    font-family:inherit; cursor:pointer; transition:.15s ease;
    text-align:left; }}
  .price-tier-row:hover {{ border-color:rgba(38,193,164,.35); }}
  .price-tier-on {{ background:rgba(38,193,164,.08);
    border-color:var(--mint);
    box-shadow:0 0 18px rgba(38,193,164,.22); }}
  ...
  .sheet-input:focus {{ border-color:var(--mint); box-shadow:0 0 0 3px rgba(38,193,164,.18); }}
  ...
  .state-status {{ display:flex; align-items:center; gap:12px;
    ...
    border-radius:16px; background:rgba(38,193,164,.08);
  ...
  .confirm-check-ring {{ position:absolute; inset:0; border-radius:999px;
    background:radial-gradient(circle, rgba(38,193,164,.30) 0%, rgba(38,193,164,0) 70%);
    filter:blur(6px); }}
  .confirm-check-inner {{ position:absolute; inset:14px; border-radius:999px;
    background:rgba(38,193,164,.10); border:2px solid rgba(38,193,164,.30);
    display:flex; align-items:center; justify-content:center;
    box-shadow:0 0 24px rgba(38,193,164,.35); }}
  ...
  .confirm-card {{ margin:8px 24px 24px; padding:18px; border-radius:24px;
    ...
    box-shadow:0 8px 24px rgba(0,0,0,{t['dim_shadow']}),
               0 0 0 1px rgba(38,193,164,.06),
               0 0 28px rgba(38,193,164,.08); }}
  ...
  .sheet-textarea {{ width:100%; padding:14px; border-radius:14px;
    ...
    transition:border-color .15s ease, box-shadow .15s ease; }}
  .sheet-textarea:focus {{ border-color:var(--mint); box-shadow:0 0 0 3px rgba(38,193,164,.18); }}
  ...
  .cal-day-on {{ background:var(--mint); color:#fff;
    box-shadow:0 4px 12px rgba(38,193,164,.35); }}
  ...
  .slot-add {{ width:100%; display:flex; align-items:center; justify-content:center;
    ...
    background:rgba(38,193,164,.06); border:1.5px dashed rgba(38,193,164,.35);
    color:var(--mint); font-family:inherit; font-size:14px; font-weight:700;
    cursor:pointer; transition:background .15s ease; }}
  .slot-add:hover {{ background:rgba(38,193,164,.12); }}
  ...
  .pcm-backdrop::after {{ top:46px; right:14px; width:170px; height:215px;
    box-shadow:6px 6px 0 -2px var(--input-border) inset;
    background:linear-gradient(135deg, rgba(38,193,164,.12), rgba(255,255,255,.04)); }}
  ...
  .pcm-sheet {{ position:absolute; left:0; right:0; bottom:0; z-index:5;
    ...
    box-shadow:0 -10px 40px rgba(0,0,0,.32),
               0 -2px 24px rgba(38,193,164,.12);
    border-top:1px solid rgba(38,193,164,.20); }}
  ...
  .pcm-row {{ display:flex; align-items:center; gap:14px;
    ...
    border:1px solid var(--input-border); }}
  .pcm-row:hover {{ border-color:rgba(38,193,164,.35);
    box-shadow:0 0 16px rgba(38,193,164,.10); }}
  ...
  .pg-recents {{ display:inline-flex; align-items:center; gap:4px;
    ...
    font-family:inherit; font-size:13px; font-weight:700; color:var(--text); }}
  .pg-cam-btn {{ width:36px; height:36px; border:1.5px solid rgba(38,193,164,.45);
    border-radius:999px; background:var(--card); color:var(--mint);
    display:flex; align-items:center; justify-content:center; cursor:pointer;
    box-shadow:0 0 14px rgba(38,193,164,.30);
    filter:drop-shadow(0 0 6px rgba(38,193,164,.30)); }}
  ...
  .pd-tag-on {{ background:rgba(38,193,164,.14); color:var(--mint);
    border:1.5px solid rgba(38,193,164,.45);
  ...
  .pd-settings {{ margin-top:24px; padding:6px 4px;
    ...
    border:1px solid rgba(38,193,164,.15);
    box-shadow:0 0 24px rgba(38,193,164,.06),
  ...
  .ps-particles {{ position:absolute; inset:0; pointer-events:none;
    background:radial-gradient(circle at 30% 25%, rgba(38,193,164,.12) 0%, transparent 28%),
               radial-gradient(circle at 75% 70%, rgba(0,179,199,.08) 0%, transparent 30%); }}
  .ps-check {{ position:relative; width:120px; height:120px;
    border-radius:999px; background:rgba(38,193,164,.10);
    display:flex; align-items:center; justify-content:center;
    box-shadow:0 0 60px rgba(38,193,164,.40),
               0 0 0 2px rgba(38,193,164,.30) inset;
    margin-bottom:24px; }}
  .ps-check-ring {{ position:absolute; inset:-12px; border-radius:999px;
    border:2px solid rgba(38,193,164,.35);
  ...
  .ps-check-icon {{ font-size:64px; color:var(--mint);
    filter:drop-shadow(0 0 10px rgba(38,193,164,.6)); }}
  ...
  .pe-stat-pill {{ flex:1; padding:10px 14px; border-radius:14px;
    background:var(--card); border:1px solid rgba(38,193,164,.18);
    box-shadow:0 0 14px rgba(38,193,164,.08); }}
